...
float calcFogIntensity(float value) {
    float x = value;
...
vec4 _linearFog(vec4 fragColor, float fragDistance, vec4 fogColor, float fogStart, float fogEnd) {
    ...
    if (fragDistance >= fogEnd) {
        factor = 1.0;
    } else {
        float maxFog = (fogEnd - fogStart);
        float intensity = calcFogIntensity((fragDistance - fogStart) / maxFog);
        fragDistance = (intensity * maxFog) + fogStart;

        factor = fragDistance < fogEnd ? smoothstep(fogStart, fogEnd, fragDistance) : 1.0; // alpha value of fog is used as a weight
    }
